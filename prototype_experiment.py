...
def write_results_df(df):
    ...
    for i, row in df.iterrows():
        df.at[i, "difficulty"] = pythia.get_difficulty(util.prefix(results_dir, row, "pythia", "prototype_part_3"))
        df.at[i, "rate_matrix"] = raxmlng.get_matrix(util.prefix(results_dir, row, "raxmlng", "prototype_part_3"))
...
def run_pythia(df):
    for (i, row) in df.iterrows():
        msa_path = row["msa_paths"]["prototype_part_3"]
        pythia.run_with_padding(msa_paths, util.prefix(results_dir, row, "pythia", "prototypei_part_3"))
...
def get_bits(i):
    ...
    binary_string = "0" * (6 - len(binary_string)) + binary_string
...
def plot_heatmaps(df):
    for i,row in df.iterrows():
        rates = raxmlng.substitution_rates(util.prefix(results_dir, row, "raxmlng", "prototype_part_3"))
        if len(rates) == 0:
            continue
        print(row["ds_id"])
...
results_dir = "data/results_prototype_part_3"
